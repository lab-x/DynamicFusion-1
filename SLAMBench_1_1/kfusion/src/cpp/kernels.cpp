...
bool non_rigid_track(float3* vertex, float3* normal, uint2 size,
...
bool test_non_rigid_track(float3* vertex, uint2 size, Matrix4 cameraMatrix, float3* inputVertex, Matrix4 pose) {
	...
	CVD::Image<u_int16_t>depthmap_n(CVD::ImageRef(size.x,size.y));
						...
						(depth[px.x + px.y * depthSize.x] - cameraX.z)
								* std::sqrt(
										1 + sq(pos.x / pos.z)
												+ sq(pos.y / pos.z));
				if (diff > -mu) {
					const float sdf = fminf(diff, mu);
					float2 data = vol[pix];
					float w = 1;
					data.x = clamp((data.y * data.x + sdf * w) / (data.y + w), -1.f,
							1.f);
					data.y = fminf(data.y + w, maxweight);
					vol.set(pix, data);
				}
			}
		}
...
bool Kfusion::tracking(float4 k, float icp_threshold, uint tracking_rate,
	...
	//reset n_warp
	for(int i = 0; i < n_warp.size(); i++) {
		n_warp[i].se3 = Eigen::Matrix4d::Identity();
	}

	bool non_rigid_return = non_rigid_track(vertex, normal, computationSize, getCameraMatrix(k), inputVertex[0], pose);
